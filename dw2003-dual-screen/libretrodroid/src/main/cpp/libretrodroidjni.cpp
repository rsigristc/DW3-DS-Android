...
JNIEXPORT jbyteArray JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_readMemory(
    ...
    try {
        if (offset < 0 || length < 0) {
            throw std::invalid_argument("Memory offset and length must be non-negative");
        }
        auto data = LibretroDroid::getInstance().readMemory(
            static_cast<unsigned int>(memoryId),
            static_cast<size_t>(offset),
            static_cast<size_t>(length)
        );
        auto result = env->NewByteArray(static_cast<jsize>(data.size()));
        if (result == nullptr) return nullptr;
        env->SetByteArrayRegion(result, 0, static_cast<jsize>(data.size()), data.data());
        return result;
    } catch (std::exception &exception) {
        LOGE("Error in readMemory: %s", exception.what());
        JavaUtils::throwRetroException(env, ERROR_GENERIC);
        return nullptr;
    }
}

JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_writeMemory(
    JNIEnv* env,
    jclass obj,
    jint memoryId,
    jint offset,
    ...
    try {
        if (offset < 0) {
            throw std::invalid_argument("Memory offset must be non-negative");
        }
        if (payload == nullptr) {
            throw std::invalid_argument("Memory write payload is required");
        }
        jboolean isCopy = JNI_FALSE;
        jsize length = env->GetArrayLength(payload);
        jbyte* data = env->GetByteArrayElements(payload, &isCopy);
        try {
            LibretroDroid::getInstance().writeMemory(
                static_cast<unsigned int>(memoryId),
                static_cast<size_t>(offset),
                data,
                static_cast<size_t>(length)
        ...
        LOGE("Error in writeMemory: %s", exception.what());
        JavaUtils::throwRetroException(env, ERROR_GENERIC);
    }
}
