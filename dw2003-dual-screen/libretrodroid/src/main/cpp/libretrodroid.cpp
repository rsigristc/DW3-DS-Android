...
std::vector<int8_t> LibretroDroid::readMemory(
    ...
    std::lock_guard<std::mutex> lock(coreLock);
    if (core == nullptr) {
        throw std::runtime_error("Cannot read memory before a core is loaded");
    }

    const size_t memorySize = core->retro_get_memory_size(memoryId);
    const auto* memory = static_cast<const int8_t*>(core->retro_get_memory_data(memoryId));
    if (memory == nullptr) {
        throw std::runtime_error("Core did not expose the requested memory region");
    }
    if (offset > memorySize || length > memorySize - offset) {
        throw std::out_of_range("Requested memory range is outside the core region");
    }
    return std::vector<int8_t>(memory + offset, memory + offset + length);
}

void LibretroDroid::writeMemory(
    unsigned int memoryId,
    size_t offset,
    const int8_t* data,
    size_t length
    ...
    std::lock_guard<std::mutex> lock(coreLock);
    if (core == nullptr) {
        throw std::runtime_error("Cannot write memory before a core is loaded");
    }
    if (data == nullptr && length > 0) {
        throw std::invalid_argument("Memory write payload is empty");
    }

    const size_t memorySize = core->retro_get_memory_size(memoryId);
    auto* memory = static_cast<int8_t*>(core->retro_get_memory_data(memoryId));
    if (memory == nullptr) {
        throw std::runtime_error("Core did not expose the requested memory region");
    }
    if (offset > memorySize || length > memorySize - offset) {
